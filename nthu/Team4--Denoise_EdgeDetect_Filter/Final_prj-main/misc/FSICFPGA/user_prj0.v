...
wire        IP_done;
reg         reg_IP_done;
reg [1:0]   output_num_state;
reg [31:0]  output_counter, output_num;
...
always @(posedge axi_clk or negedge axi_reset_n)  begin
  if ( !axi_reset_n ) begin
    ...
    if ( !IP_done && awvalid_in && wvalid_in ) begin        //when awvalid_in=1 and wvalid_in=1 means awready_out=1 and wready_out=1 
...
always @(posedge axi_clk or negedge axi_reset_n)  begin
  if ( !axi_reset_n ) begin
    ...
    if (awvalid_in && wvalid_in && awaddr[11:2] == 10'h001) output_num_state[0] <= 1;
    else if(awvalid_in && wvalid_in && awaddr[11:2] == 10'h002) output_num_state[1] <= 1;
    else if (output_num_state==2'b11) output_num_state <= 2'b0;
    else output_num_state <= output_num_state;
  end
end
...
always @(posedge axi_clk or negedge axi_reset_n)  begin
  if ( !axi_reset_n ) begin
    ...
    if (output_num_state==2'b11) output_num <= reg_widthIn * reg_heightIn;
    else if(IP_done) output_num <= 0;
...
always @(posedge axi_clk or negedge axi_reset_n)  begin
  if ( !axi_reset_n ) begin
    ...
    if (sm_tvalid && sm_tready) output_counter <= output_counter + 1;
    else output_counter <= output_counter;
  end
end

assign IP_done = output_counter==output_num;
assign sm_tlast = (sm_tvalid && sm_tready && output_counter==output_num-1);
