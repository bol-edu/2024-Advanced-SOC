...
int ParseIspCfgFile(const std::string cfg_file_path, IspPrms &isp_prm)
{
    ...
    if (((int)pwl_x.size() != isp_prm.depwl_prm.pwl_nums) || ((int)pwl_y.size() != isp_prm.depwl_prm.pwl_nums) || ((int)pwl_slope.size() != isp_prm.depwl_prm.pwl_nums))
    {
    ...
    if ((int)gamma_curve.size() != isp_prm.rgb_gamma.nums)
    {
