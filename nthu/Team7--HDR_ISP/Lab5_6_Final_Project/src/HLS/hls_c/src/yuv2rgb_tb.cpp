...
#include "/home/course/ee525201/final_project/ISP1/hls_c/inc/yuv2rgb.h"
...
#include "/home/course/ee525201/final_project/ISP1/cmodel/inc/srcs/sources/modules/saturation.h"
#include "/home/course/ee525201/final_project/ISP1/cmodel/inc/srcs/sources/modules/yuv2rgb.h"
#include "/home/course/ee525201/final_project/ISP1/cmodel/inc/srcs/sources/modules/yuv2rgb.cpp"
...
{	
	...
	saturation_algorithm<iW, iH> ref_saturation_inst;
	yuv2bgr_algorithm<iW, iH> ref_yuv2bgr_inst;
	//HDRISP_IP::unpack u_unpack;
	//HDRISP_IP::depwl u_depwl;
	//HDRISP_IP::lsc u_lsc;
	//HDRISP_IP::saturation u_saturation;
	HDRISP_IP::yuv2bgr u_yuv2bgr;
	...
	ref_saturation_inst.Saturation(&frame, &isp_prms);
	ref_yuv2bgr_inst.Yuv2Bgr(&frame, &isp_prms);
	//dpc.cpp
	...
	ref_cns_inst.Cns(&frame2, &isp_prms);
	ref_saturation_inst.Saturation(&frame2, &isp_prms);
	Yuv2Bgr(&frame2, &isp_prms);
	...
	ref_cns_inst.Cns(&frame3, &isp_prms);
	ref_saturation_inst.Saturation(&frame3, &isp_prms);
	...
	u_yuv2bgr.run(dataIn_chn, widthIn, heightIn, dataOut_chn);
	...
	data1_bgr = (uint8_t*)frame.data.bgr_u8_o;
	data2_bgr = (uint8_t*)frame2.data.bgr_u8_o;
	
	for(int i = 0; i < iH; i++){
		for(int j = 0; j < iW; j++){
			
		  if((int)data1_bgr[(i * iW + j) * 3 + 0] != (int)data2_bgr[(i * iW + j) * 3 + 0]){
  	        err_num += 1;
		  }
		  
          if((int)data1_bgr[(i * iW + j) * 3 + 1] != (int)data2_bgr[(i * iW + j) * 3 + 1]){
  	        err_num += 1;
		  }
		  
 		  if((int)data1_bgr[(i * iW + j) * 3 + 2] != (int)data2_bgr[(i * iW + j) * 3 + 2]){
  	        err_num += 1;
		  }
	...
	uint8 data_hls_out_r = 0;
	uint8 data_hls_out_g = 0;
	uint8 data_hls_out_b = 0;
	int comp_thres = 1;
	for(int i = 0; i < iH; i++){
		for(int j = 0; j < iW; j++){
			
		  data_hls_out = dataOut_chn.read();
		  data_hls_out_r = data_hls_out.slc<8>(16);
		  data_hls_out_g = data_hls_out.slc<8>(8);
		  //std::cout << "data_hls_out_u = " << (unsigned)((uint8_t)data_hls_out_u) << std::endl;
		  data_hls_out_b = data_hls_out.slc<8>(0);
		  
		  
		  if(abs((int)data_hls_out_b - (int)data2_bgr[(i * iW + j) * 3 + 0]) > comp_thres){
			//if(i * iW + j < 7680){
  	          std::cout << "B: No." << (i * iW + j)  << " = " << (int)data_hls_out_b 
  	                    << ", data2 = " << (int)data2_bgr[(i * iW + j) * 3 + 0] << std::endl;
  	        //}
			hls_err_num += 1;
		  }
			
		  
		  if(abs((int)data_hls_out_g - (int)data2_bgr[(i * iW + j) * 3 + 1]) > comp_thres){
			hls_err_num += 1;
		  }
		  
		  
		  if(abs((int)data_hls_out_r - (int)data2_bgr[(i * iW + j) * 3 + 2]) > comp_thres){
			hls_err_num += 1;
		  }
