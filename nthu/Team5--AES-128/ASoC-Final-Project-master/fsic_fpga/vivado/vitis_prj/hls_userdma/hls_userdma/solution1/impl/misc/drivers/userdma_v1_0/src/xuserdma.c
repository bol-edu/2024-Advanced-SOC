...
void XUserdma_Set_endianness(XUserdma *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XUserdma_WriteReg(InstancePtr->Control_BaseAddress, XUSERDMA_CONTROL_ADDR_ENDIANNESS_DATA, Data);
}

u32 XUserdma_Get_endianness(XUserdma *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XUserdma_ReadReg(InstancePtr->Control_BaseAddress, XUSERDMA_CONTROL_ADDR_ENDIANNESS_DATA);
    return Data;
}
