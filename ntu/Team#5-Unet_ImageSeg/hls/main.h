...
class main{
    ...
    //conv2d_inst.run(buf1, padded_input, buf2, 64, 64, 3, 1, 0, 3, 8);
    conv2d_inst.run(buf1, padded_input, buf2, height, width, kernel_size, padding, filter_offset, in_channels, out_channels);
    //batchnorm_relu : input, output, gamma, beta, channels, height, width, offset
    //batchnorm_relu_inst.run(buf2, buf1, 8, 64, 64, 0);
    batchnorm_relu_inst.run(buf2, buf1, out_channels, height, width, gamma_offset);
    filter_offset += 8*3*3*3;
    gamma_offset += 8;

    // conv2d_inst.run(buf1, padded_input, buf2, 64, 64, 3, 1, filter_offset, 8, 8);
    // batchnorm_relu_inst.run(buf2, enc1, 8, 64, 64, gamma_offset);
    conv2d_inst.run(buf1, padded_input, buf2, height, width, kernel_size, padding, filter_offset, out_channels, out_channels);
    batchnorm_relu_inst.run(buf2, enc1, out_channels, height, width, gamma_offset);
    filter_offset += 8*8*3*3;
    gamma_offset += 8;
    //maxpool : input, output,channels, height, width,  pool_size, stride
    maxpool_inst.run(enc1, buf1, out_channels, height, width, pool_size, stride);
    ...
    conv2d_inst.run(buf1, padded_input, buf2, height, width, kernel_size, padding, filter_offset, out_channels, out_channels);
    batchnorm_relu_inst.run(buf2, enc2, out_channels, height, width, gamma_offset);
    filter_offset += 16*16*3*3;
    gamma_offset += 16;
    maxpool_inst.run(enc2, buf1, out_channels, height, width, pool_size, stride);
    ...
    conv2d_inst.run(buf1, padded_input, buf2, height, width, kernel_size, padding, filter_offset, out_channels, out_channels);
    batchnorm_relu_inst.run(buf2, enc3, out_channels, height, width, gamma_offset);
    filter_offset += 32*32*3*3;
    gamma_offset += 32;
    maxpool_inst.run(enc3, buf1, out_channels, height, width, pool_size, stride);
    ...
    conv2d_inst.run(buf1, padded_input, buf2, height, width, kernel_size, padding, filter_offset, out_channels, out_channels);
    batchnorm_relu_inst.run(buf2, enc4, out_channels, height, width, gamma_offset);
    filter_offset += 64*64*3*3;
    gamma_offset += 64;
    maxpool_inst.run(enc4, buf1, out_channels, height, width, pool_size, stride);
    ...
    tranconv2d_inst.run(buf1, buf2, height, width, kernel_size_trans, filter_offset, in_channels, out_channels, stride);
    ...
    conv2d_inst.run(buf2, padded_input, buf1, height, width, kernel_size, padding, filter_offset, in_channels, out_channels);
    batchnorm_relu_inst.run(buf1, buf2, out_channels, height, width, gamma_offset);
    filter_offset += 64*128*3*3;
    gamma_offset += 64;

    conv2d_inst.run(buf2, padded_input, buf1, height, width, kernel_size, padding, filter_offset, out_channels, out_channels);
    batchnorm_relu_inst.run(buf1, buf2, out_channels, height, width, gamma_offset);
    filter_offset += 64*64*3*3;
    gamma_offset += 64;
    ...
    //dec3
    tranconv2d_inst.run(buf2, buf1, height, width, kernel_size_trans, filter_offset, in_channels, out_channels, stride);
