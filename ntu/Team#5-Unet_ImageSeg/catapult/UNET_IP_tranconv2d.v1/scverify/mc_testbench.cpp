...
mc_wait_ctrl testbench::output_IN_wait_ctrl;
...
void testbench::exec_run_tran(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &height, ac_int<7, false> &width, ac_int<2, false> &kernel_size, ac_int<20, false> &filter_offset, ac_int<7, false> &in_channels, ac_int<7, false> &out_channels, ac_int<3, false> &stride) {
   return mc_testbench::exec_run_tran(input, output, height, width, kernel_size, filter_offset, in_channels, out_channels, stride);
}
#endif
...
// ============================================
// Function: capture_output_IN
// --------------------------------------------

void mc_testbench::capture_output_IN( ac_fixed<12, 8, true, AC_TRN, AC_WRAP > output[32768])
{
   if (output_IN_capture_count == wait_cnt)
      wait_on_input_required();
   if (_capture_output_IN && !testbench::output_ignore)
   {
      int cur_iter = output_IN_iteration_count;
      ++output_IN_iteration_count;
      mgc_sysc_ver_array1D<ac_fixed<12, 8, true, AC_TRN, AC_WRAP >,32768> output_IN_tmp;
      int output_linear_idx = 0;
      for (int output_idx_0 = 0; output_idx_0 < 32768; ++output_idx_0)
         output_IN_tmp[output_linear_idx++] = output[output_idx_0];
      ccs_output_IN->put(output_IN_tmp);
      ++output_IN_capture_count;
      mc_testbench_util::process_wait_ctrl("output_IN",testbench::output_IN_wait_ctrl,ccs_wait_ctrl_output_IN.operator->(),cur_iter,output_IN_capture_count,0);
   }
...
void mc_testbench::capture_kernel_size( ac_int<2, false > &kernel_size)
{
   ...
   if (_capture_kernel_size && !testbench::kernel_size_ignore)
   {
      ...
      ccs_kernel_size->put(kernel_size); // THIS
...
void mc_testbench::capture_filter_offset( ac_int<20, false > &filter_offset)
{
   ...
   if (_capture_filter_offset && !testbench::filter_offset_ignore)
   {
      ...
      ccs_filter_offset->put(filter_offset); // THIS
...
void mc_testbench::capture_in_channels( ac_int<7, false > &in_channels)
{
   ...
   if (_capture_in_channels && !testbench::in_channels_ignore)
   {
      ...
      ccs_in_channels->put(in_channels); // THIS
...
void mc_testbench::capture_out_channels( ac_int<7, false > &out_channels)
{
   ...
   if (_capture_out_channels && !testbench::out_channels_ignore)
   {
      ...
      ccs_out_channels->put(out_channels); // THIS
...
void mc_testbench::wait_on_input_required()
{
   ...
   while (atleast_one_active_input) {
      ...
      if (_capture_output_IN && ccs_output_IN->used() == 0) return;
      if (_capture_height && ccs_height->used() == 0) return;
      if (_capture_width && ccs_width->used() == 0) return;
      if (_capture_kernel_size && ccs_kernel_size->used() == 0) return;
      if (_capture_filter_offset && ccs_filter_offset->used() == 0) return;
      if (_capture_in_channels && ccs_in_channels->used() == 0) return;
      if (_capture_out_channels && ccs_out_channels->used() == 0) return;
      if (_capture_stride && ccs_stride->used() == 0) return;
      that->cpp_testbench_active.write(false);
      if (average_period > SC_ZERO_TIME && sc_time_stamp() != timeout)
         wait(average_period * 10, ccs_input->ok_to_put() | ccs_output_IN->ok_to_put() | ccs_height->ok_to_put() | ccs_width->ok_to_put() | ccs_kernel_size->ok_to_put() | ccs_filter_offset->ok_to_put() | ccs_in_channels->ok_to_put() | ccs_out_channels->ok_to_put() | ccs_stride->ok_to_put());
      else
         wait(ccs_input->ok_to_put() | ccs_output_IN->ok_to_put() | ccs_height->ok_to_put() | ccs_width->ok_to_put() | ccs_kernel_size->ok_to_put() | ccs_filter_offset->ok_to_put() | ccs_in_channels->ok_to_put() | ccs_out_channels->ok_to_put() | ccs_stride->ok_to_put());
      that->cpp_testbench_active.write(true);
      if (timed_out()) {
...
void mc_testbench::capture_IN(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &height, ac_int<7, false> &width, ac_int<2, false> &kernel_size, ac_int<20, false> &filter_offset, ac_int<7, false> &in_channels, ac_int<7, false> &out_channels, ac_int<3, false> &stride)
{
   that->capture_input(input);
   that->capture_output_IN(output);
...
void mc_testbench::capture_OUT(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &height, ac_int<7, false> &width, ac_int<2, false> &kernel_size, ac_int<20, false> &filter_offset, ac_int<7, false> &in_channels, ac_int<7, false> &out_channels, ac_int<3, false> &stride)
{
   that->capture_output(output);
}
// ============================================
// Function: exec_run_tran
// --------------------------------------------

void mc_testbench::exec_run_tran(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &height, ac_int<7, false> &width, ac_int<2, false> &kernel_size, ac_int<20, false> &filter_offset, ac_int<7, false> &in_channels, ac_int<7, false> &out_channels, ac_int<3, false> &stride)
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   wait_for_idle_sync();
   capture_IN(input, output, height, width, kernel_size, filter_offset, in_channels, out_channels, stride);
   UNET_IP::tranconv2d::run_tran(input, output, height, width, kernel_size, filter_offset, in_channels, out_channels, stride);
   ...
   capture_OUT(input, output, height, width, kernel_size, filter_offset, in_channels, out_channels, stride);
   #else
   #endif
}
// ============================================
...
void mc_testbench::run() {
   ...
   testbench::output_IN_wait_ctrl.clear();
   output_IN_capture_count = 0;
   output_IN_iteration_count = 0;
   testbench::output_use_mask = false;
   ...
   cout<<"   captured "<<output_IN_capture_count<<" values of output_IN"<<endl;
