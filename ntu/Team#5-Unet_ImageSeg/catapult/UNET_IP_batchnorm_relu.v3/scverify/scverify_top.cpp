...
public:
  ...
  sc_signal<sc_lv<10> >                                                                              TLS_offset;
  ccs_DUT_wrapper                                                                                    UNET_IP_batchnorm_relu_INST;
  ...
  mgc_in_wire_trans_rsc_v1<1,10 >                                                                    offset_rsc_INST;
  ...
  tlm::tlm_fifo<ac_int<10, false > >                                                                 TLS_in_fifo_offset;
  tlm::tlm_fifo<mc_wait_ctrl>                                                                        TLS_in_wait_ctrl_fifo_offset;
  mc_input_transactor<ac_int<10, false >,10,false>                                                   transactor_offset;
  ...
  scverify_top(const sc_module_name& name)
    ...
    , UNET_IP_batchnorm_relu_INST("rtl", "ccs_wrapper")
    ...
    , transactor_offset("transactor_offset", 0, 10, 0, d_disable_on_empty)
    ...
    , sync_generator_INST("sync_generator", true, false, false, false, 4001, 4001, 0)
    , catapult_monitor_INST("Monitor", clk, true, 4001LL, 4001LL)
    ...
    , deadlocked("deadlocked")
  {
    ...
    UNET_IP_batchnorm_relu_INST.clk(clk);
    UNET_IP_batchnorm_relu_INST.rst(TLS_rst);
    UNET_IP_batchnorm_relu_INST.arst_n(TLS_arst_n);
    UNET_IP_batchnorm_relu_INST.input_rsc_radr(TLS_input_rsc_radr);
    UNET_IP_batchnorm_relu_INST.input_rsc_re(TLS_input_rsc_re);
    UNET_IP_batchnorm_relu_INST.input_rsc_q(TLS_input_rsc_q);
    UNET_IP_batchnorm_relu_INST.input_rsc_clken(TLS_input_rsc_clken);
    UNET_IP_batchnorm_relu_INST.input_triosy_lz(TLS_input_triosy_lz);
    UNET_IP_batchnorm_relu_INST.output_rsc_wadr(TLS_output_rsc_wadr);
    UNET_IP_batchnorm_relu_INST.output_rsc_d(TLS_output_rsc_d);
    UNET_IP_batchnorm_relu_INST.output_rsc_we(TLS_output_rsc_we);
    UNET_IP_batchnorm_relu_INST.output_rsc_clken(TLS_output_rsc_clken);
    UNET_IP_batchnorm_relu_INST.output_triosy_lz(TLS_output_triosy_lz);
    UNET_IP_batchnorm_relu_INST.channels(TLS_channels);
    UNET_IP_batchnorm_relu_INST.height(TLS_height);
    UNET_IP_batchnorm_relu_INST.width(TLS_width);
    UNET_IP_batchnorm_relu_INST.offset(TLS_offset);
    ...
    offset_rsc_INST.z(TLS_offset);
    offset_rsc_INST.add_attribute(*(new sc_attribute<double>("CLK_SKEW_DELAY", __scv_hold_time_RSCID_6)));
    ...
    transactor_offset.set_disable_on_empty(d_disable_on_empty);
    transactor_offset.bind_clk(clk, true, rst);
    transactor_offset.add_attribute(*(new sc_attribute<int>("MC_TRANSACTOR_EVENT", 0 )));
    if (!d_idle_sync_enabled) { transactor_offset.register_block(&offset_rsc_INST, offset_rsc_INST.basename(), in_sync, 0, 0, 1); }
    if (d_idle_sync_enabled) { transactor_offset.register_block(&offset_rsc_INST, offset_rsc_INST.basename(), TLS_design_is_idle, 0, 0, 1); }
...
void scverify_top::read_env() {
  ...
  unsigned long latency_est = 3997;
...
void scverify_top::setup_autowait() {
  ...
  transactor_offset.set_auto_wait_limit(env_SCVerify_AUTOWAIT);
  transactor_offset.configure_autowait(autowait_input_cfg);
}
...
void scverify_top::inform_autowait() {
  ...
  waited |= (transactor_offset.atleast_one_autowait(0) > 0);
  if (waited)
...
void scverify_top::setup_debug() {
  ...
  static int transactor_offset_flags = 0;
#endif
