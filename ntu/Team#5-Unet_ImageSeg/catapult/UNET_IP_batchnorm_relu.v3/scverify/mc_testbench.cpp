...
void testbench::exec_run_batch(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &channels, ac_int<7, false> &height, ac_int<7, false> &width, ac_int<10, false> &offset) {
   return mc_testbench::exec_run_batch(input, output, channels, height, width, offset);
}
#endif
...
void mc_testbench::capture_offset( ac_int<10, false > &offset)
{
   ...
   if (_capture_offset && !testbench::offset_ignore)
   {
      ...
      ccs_offset->nb_put(offset); // THIS
      ++offset_capture_count;
      if (offset_iteration_count > 1 && !offset_mismatch && offset_prev_value != offset)
         offset_mismatch = true;
      offset_prev_value = offset;
      if (!testbench::enable_idle_sync_mode && offset_mismatch && offset_idle_warning) {
         offset_idle_warning = false;
         std::ostringstream msg; msg.str("");
         msg << "Direct input \"offset\" had its value changed between design function calls with idle synchronization disabled.\n";
         msg << "Warning:  Potential simulation mismatch detected. Consider setting \"IDLE_SYNCHRONIZATION_MODE\" to true in the SCVerify flow.\n";
         msg << "Warning:  Further warnings for this direct input will be suppressed.";
         SC_REPORT_WARNING("User testbench", msg.str().c_str());
      }
...
void mc_testbench::capture_IN(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &channels, ac_int<7, false> &height, ac_int<7, false> &width, ac_int<10, false> &offset)
{
...
void mc_testbench::capture_OUT(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &channels, ac_int<7, false> &height, ac_int<7, false> &width, ac_int<10, false> &offset)
{
   that->capture_output(output);
}
// ============================================
// Function: exec_run_batch
// --------------------------------------------

void mc_testbench::exec_run_batch(ac_fixed<12, 8, true, AC_TRN, AC_WRAP> input[32768], ac_fixed<12, 8, true, AC_TRN, AC_WRAP> output[32768], ac_int<7, false> &channels, ac_int<7, false> &height, ac_int<7, false> &width, ac_int<10, false> &offset)
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   wait_for_idle_sync();
   capture_IN(input, output, channels, height, width, offset);
   UNET_IP::batchnorm_relu::run_batch(input, output, channels, height, width, offset);
   ...
   capture_OUT(input, output, channels, height, width, offset);
   #else
   #endif
}
// ============================================
